...
import { normalizeStatus as canonicalize } from './lib/status.mjs';
...
/**
 * Wrap canonicalize() to also flag DUPLICADO/Repost rows for "moveToNotes"
 * (preserving the original raw text in the notes column) and to flag empty
 * statuses as Discarded. Returns { status, moveToNotes?, unknown? }.
 */
function normalizeStatus(raw) {
  ...
  // DUPLICADO and Repost: canonicalize → Discarded but also preserve the
  // raw text in the notes column for traceability.
  if (/^duplicado/i.test(trimmed) || /^dup\b/i.test(trimmed) || /^repost/i.test(trimmed)) {
