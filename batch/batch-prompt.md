...
**RULE: NEVER hardcode metrics.** Read them from cv.md + article-digest.md at the time of evaluation.
...
> **Concrete metrics are read from `cv.md` + `article-digest.md` for every evaluation. NEVER hardcode numbers here.**
...
**Cross-cutting advantage**: Frame the profile as **"Technical builder"** that adapts framing to the role:
...
Read `cv.md`. Build a table mapping every JD requirement to exact lines from the CV or i18n.ts keys.
...
2. **"Sell senior without lying" plan**: specific phrasing, concrete achievements, founder background as an advantage
3. **"If they down-level me" plan**: accept if comp is fair, 6-month review, clear criteria

#### Block D -- Comp and Demand

Use WebSearch for current salaries (Glassdoor, Levels.fyi, Blind), the company's comp reputation, role demand trend. Table with data and cited sources. If no data is available, say so.
...
11. Build the full HTML from the template (read `templates/cv-template.html`)
...
- Header: Space Grotesk 24px bold + 2px cyan→purple gradient + contact
