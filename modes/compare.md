...
For each offer: per-dimension score, weighted total score.
Final ranking + recommendation with time-to-offer considerations.

If the offers are not in context, ask the user for them. They can be text, URLs, or references to offers already evaluated in the tracker.
